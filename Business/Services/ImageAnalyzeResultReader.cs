...
{
    public class ImageAnalyzeResultReader : IImageAnalyzeResultReader
    {
        private readonly IRestClient _client;
        private readonly CognitiveServicesConfig _config;
        private readonly ITaskProxy _task;

        public ImageAnalyzeResultReader(IRestClient client, IConfiguration configuration, ITaskProxy task)
        {
            _client = client;
            _task = task;
        ...
        public async Task<HsaResult> GetAnalyzeResultAsync(string operationLocationAddress)
        {
            ...
            while (true)
            {
                ...
                {
                    return analyzeResultResponse.Data;
                }
        ...
        {
            var delayMilliseconds = Math.Pow(2, numberOfRetries - 1) * 1000;
